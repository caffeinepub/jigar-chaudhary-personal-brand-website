...
export default function Navigation() {
  ...
  const handleZindagiClick = () => {
    setMobileOpen(false);
    window.location.hash = "#/zindagi30";
  };
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-16 md:h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-5 lg:gap-7">
            ...
            <button
              type="button"
              onClick={handleZindagiClick}
              className="font-body text-sm font-semibold text-brand-light/70 border border-white/20 px-4 py-1.5 rounded-full hover:bg-white/10 hover:text-brand-light hover:border-white/40 transition-all duration-200 tracking-wide"
            >
              ✨ Zindagi 3.0
            </button>
          </div>
      ...
      >
        <div className="flex flex-col items-center gap-8">
          {navLinks.map((link, i) => (
            <button
              ...
              style={{
                ...
                transition:
                  "transform 0.3s ease, opacity 0.3s ease, color 0.2s ease",
              }}
            >
              {link.label}
            </button>
          ...
          {/* Zindagi 3.0 mobile link */}
          <button
            type="button"
            onClick={handleZindagiClick}
            className="font-body font-semibold text-brand-light/80 border border-white/20 px-6 py-2.5 rounded-full text-lg tracking-wide hover:bg-white/10 transition-colors duration-200"
            style={{
              transitionDelay: mobileOpen
                ? `${(navLinks.length + 1) * 60}ms`
                : "0ms",
              transform: mobileOpen ? "translateY(0)" : "translateY(16px)",
              opacity: mobileOpen ? 1 : 0,
              transition: "transform 0.3s ease, opacity 0.3s ease",
          ...
          <button
            ...
            style={{
              transitionDelay: mobileOpen
                ? `${(navLinks.length + 2) * 60}ms`
                : "0ms",
              transform: mobileOpen ? "translateY(0)" : "translateY(16px)",
              opacity: mobileOpen ? 1 : 0,
              transition:
                "transform 0.3s ease, opacity 0.3s ease, background-color 0.2s ease",
            }}
          >
            BOOK NOW
          </button>
