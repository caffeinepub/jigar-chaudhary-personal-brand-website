...
import { ArrowLeft, Radio, CheckCircle, Users, Clock, MapPin, Calendar, Star, Loader2, Zap, Brain, Target, TrendingUp } from 'lucide-react';
...
function CountdownBox({ value, label }: { value: number; label: string }) {
  return (
    <div className="flex flex-col items-center bg-reg-card border border-reg-gold/20 rounded-2xl px-4 py-4 min-w-[72px]">
      ...
      <span className="text-reg-muted text-xs uppercase tracking-widest mt-1">{label}</span>
    </div>
  );
}

export default function RegisterPage() {
  const [form, setForm] = useState({ name: '', email: '', phone: '', collegeProfession: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.name.trim() || !form.email.trim() || !form.phone.trim() || !form.collegeProfession.trim()) {
    ...
    try {
      ...
      if (msg.includes('Actor not available') || msg.includes('Actor not initialized')) {
    ...
    return (
      <div className="min-h-screen bg-reg-dark flex items-center justify-center p-4">
        <div className="text-center max-w-lg w-full">
          ...
          <h2 className="text-4xl font-display text-white mb-3">You're Registered!</h2>
          <p className="text-gray-400 mb-8">
            Thank you, <span className="text-reg-gold font-semibold">{form.name}</span>! Your spot for the live session is confirmed. We'll send details to <span className="text-white">{form.email}</span>.
          </p>
          ...
          <div className="bg-reg-card border border-reg-gold/20 rounded-2xl p-4 mb-8 flex items-center justify-center gap-3">
            ...
            <div className="text-left">
              <p className="text-reg-muted text-xs uppercase tracking-widest">Session Date</p>
              <p className="text-white font-semibold text-sm">Sunday, 8th March 2026 · 10:00 AM – 12:00 PM IST</p>
            </div>
          </div>
          ...
          <div className="bg-green-950/60 border-2 border-green-500/50 rounded-2xl p-6 mb-8">
            <div className="flex items-center justify-center gap-2 mb-3">
              ...
              <h3 className="text-xl font-display text-white">Join Our WhatsApp Community</h3>
            </div>
            <p className="text-green-300 text-sm mb-1 font-semibold">
              ⚠️ It's mandatory to join the community!
            </p>
            <p className="text-gray-400 text-sm mb-5">
              All session updates, links, and resources will be shared exclusively in the WhatsApp community. Don't miss out!
            </p>
            <a
              href={WHATSAPP_LINK}
              target="_blank"
    ...
    <div className="min-h-screen bg-reg-dark text-white">
      ...
      <section className="relative min-h-[70vh] flex items-center justify-center overflow-hidden">
        <div
          ...
          style={{ backgroundImage: 'url(/assets/generated/register-hero-bg.dim_1440x900.png)' }}
        />
        ...
        <div className="relative z-10 text-center px-4 max-w-4xl mx-auto py-16">
          <div className="inline-flex items-center gap-2 bg-reg-gold/10 border border-reg-gold/30 rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-reg-gold text-sm font-medium tracking-wide uppercase">Live Session</span>
          </div>
          ...
          <p className="text-lg md:text-xl text-gray-300 mb-6 max-w-2xl mx-auto">
            Join Jigar Chaudhary for an exclusive live session on transforming pressure into peak performance.
          </p>
          ...
          <div className="inline-flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-5 py-2.5 mb-8">
            ...
            <span className="text-white font-medium text-sm">Sunday, 8th March 2026 · 10:00 AM – 12:00 PM IST</span>
          </div>
          ...
          <div className="mb-8">
            {countdown.isLive ? (
              <div className="inline-flex items-center gap-2 bg-green-500/20 border border-green-500/40 rounded-full px-6 py-3">
                ...
                <span className="text-green-400 font-bold text-lg tracking-wide">SESSION IS LIVE NOW!</span>
              </div>
            ) : (
              <div>
                <p className="text-reg-muted text-xs uppercase tracking-widest mb-3">Session starts in</p>
                <div className="flex items-center justify-center gap-3">
                  <CountdownBox value={countdown.days} label="Days" />
                  <span className="text-reg-gold text-2xl font-display mb-4">:</span>
      ...
      <section className="py-12 px-4 max-w-5xl mx-auto">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          {[
            ...
            <div key={label} className="bg-reg-card border border-white/5 rounded-2xl p-5 text-center">
              <Icon className="w-6 h-6 text-reg-gold mx-auto mb-2" />
              <p className="text-reg-muted text-xs uppercase tracking-widest mb-1">{label}</p>
          ...
          <div className="grid md:grid-cols-2 gap-0">
            ...
            <div className="p-8 flex flex-col justify-center">
              <div className="inline-flex items-center gap-2 bg-reg-gold/10 border border-reg-gold/20 rounded-full px-3 py-1 mb-4 w-fit">
                <span className="text-reg-gold text-xs font-medium uppercase tracking-widest">Your Host</span>
              </div>
              <h2 className="text-3xl font-display text-white mb-2">JIGAR CHAUDHARY</h2>
              <p className="text-reg-gold text-sm font-medium mb-4">Youth Trainer & Experiential Facilitator</p>
              <p className="text-gray-400 text-sm leading-relaxed mb-6">
                Jigar Chaudhary has transformed thousands of lives through his high-energy sessions on mindset, performance, and personal growth. With a unique blend of practical tools and motivational storytelling, he helps students and professionals unlock their true potential.
              </p>
              <div className="flex flex-wrap gap-3">
                {['130+ Sessions', '50,000+ Lives Impacted', 'Pan-India Reach'].map(tag => (
                  <span key={tag} className="bg-reg-dark border border-white/10 rounded-full px-3 py-1 text-xs text-gray-300">
                    {tag}
                  </span>
                ))}
              </div>
        ...
        {/* What You'll Learn */}
        <div className="mb-16">
          <h2 className="text-3xl font-display text-white mb-2 text-center">WHAT YOU'LL LEARN</h2>
          <p className="text-reg-muted text-center text-sm mb-8">Practical insights you can apply immediately</p>
          <div className="grid md:grid-cols-2 gap-4">
            {[
              { icon: Brain, text: 'How to reframe pressure as a performance catalyst' },
              { icon: Zap, text: 'Mindset techniques used by top performers worldwide' },
              { icon: Target, text: 'Practical tools to stay calm and focused under stress' },
              { icon: TrendingUp, text: 'Building mental resilience for long-term success' },
              { icon: Star, text: 'Live Q&A session with Jigar Chaudhary' },
              { icon: Users, text: 'Exclusive resources, worksheets & action plan' },
            ].map(({ icon: Icon, text }) => (
              <div key={text} className="flex items-start gap-3 bg-reg-card border border-white/5 rounded-xl p-4 hover:border-reg-gold/20 transition-colors">
        ...
        <div className="mb-16 bg-green-950/40 border border-green-500/30 rounded-2xl p-6 flex flex-col md:flex-row items-center gap-4">
          ...
          <div className="text-center md:text-left">
            <p className="text-white font-semibold mb-1">Join Our WhatsApp Community — Mandatory!</p>
            <p className="text-gray-400 text-sm">After registration, you'll receive a link to join our exclusive WhatsApp community where all session updates and resources will be shared.</p>
          </div>
        </div>
        ...
        <div ref={formRef} id="register-form" className="max-w-lg mx-auto">
          <div className="bg-reg-card border border-white/10 rounded-3xl p-8">
            <div className="text-center mb-6">
              <h2 className="text-2xl font-display text-white mb-2">SECURE YOUR SPOT</h2>
              <p className="text-reg-muted text-sm">Fill in your details to register for the live session</p>
              <div className="mt-3 inline-flex items-center gap-2 bg-reg-gold/10 border border-reg-gold/20 rounded-full px-3 py-1">
                ...
                <span className="text-reg-gold text-xs font-medium">8th March 2026 · 10:00 AM – 12:00 PM IST</span>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {[
                ...
                { name: 'collegeProfession', label: 'College / Profession', placeholder: 'e.g. Student at XYZ College', type: 'text' },
              ].map(({ name, label, placeholder, type }) => (
                <div key={name}>
                  <label className="block text-sm text-reg-muted mb-1.5">{label}</label>
                  <input
                    type={type}
                    name={name}
              ...
              {errorMsg && (
                <p className="text-red-400 text-sm text-center bg-red-500/10 border border-red-500/20 rounded-lg px-4 py-2">{errorMsg}</p>
              )}
