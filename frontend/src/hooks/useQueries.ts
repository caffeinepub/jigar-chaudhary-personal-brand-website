...
export function useSubmitBooking() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      return actor.submitBooking(data.name, data.email, data.phone, data.organization, data.eventType, data.message);
    },
    onSuccess: () => {
...
export function useSubmitRegistration() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (!resolvedActor) {
        resolvedActor = await new Promise<backendInterface>((resolve, reject) => {
          let attempts = 0;
          const interval = setInterval(() => {
            ...
            if (current) {
              ...
              clearInterval(interval);
              reject(new Error('Actor not available. Please refresh and try again.'));
            }
          }, 100);
        });
      }
